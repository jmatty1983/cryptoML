...
{   
    ...
    if (candles.length > smaLength){
        ...
        tulind.indicators.sma.indicator([closed.slice(closed.length - smaLength)], [smaLength], function(err, results) {
            sma = Math.floor(results[0] * 100) / 100.0;
          });
